...
function createCalculator() {
  ...
  // TODO: Add types to object params
  ...
  function onDisplayChange(mutations: MutationRecord[]) {
    try {
  ...
  function doCalc(a: string | number, b: string | number, operator) {
    try {
      a = typeof a === 'string' ? parseFloat(a) : a
      b = typeof b === 'string' ? parseFloat(b) : b
      if (isNaN(a) || isNaN(b)) {
        throw Error('Something went wrong! not a number!')
      }
  ...
  function getDisplayValue() {
    return parseFloat(displayTextElem.innerText)
  }

  // Update the display with a new value.
  function updateDisplay(val = 0) {
    displayTextElem.innerHTML = String(val)
  }
  ...
  // Handle number keys
  function onNumberChange(event: MouseEvent) {
    const { cleared } = calcState
    const startingValue = getDisplayValue()
    const valueToAdd = (event.currentTarget as HTMLButtonElement).innerText
    if (calcState.clearToType) {
      updateDisplay(concatDisplayValues(startingValue, valueToAdd))
    } else {
      updateDisplay(parseFloat(valueToAdd))
  ...
  function activateMath(cur: HTMLButtonElement) {
    removeActiveMath()
  ...
  function onPressDecimal() {
    const curVal = String(getDisplayValue())
