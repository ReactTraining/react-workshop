...
export const Accordion = React.forwardRef(
  (
    ...
    const inc = React.useRef(-1)

    const context = {
      accordionId,
      registerIndex: (ref) => {
        if (!ref.current) {
          ref.current = inc.current = inc.current + 1
        }
        return ref.current
      },
...
export const AccordionItem = React.forwardRef(({ children, ...props }, forwardedRef) => {
  ...
  const context = {
    ...
    buttonId,
    selected,
    itemSelectPanel: () => selectPanel(index),
  }
...
export const AccordionButton = React.forwardRef(({ children, onClick, ...props }, forwardedRef) => {
  const { buttonId, panelId, selected, itemSelectPanel } = React.useContext(AccordionItemContext)

  return (
    <button
      {...props}
      onClick={wrapEvent(onClick, itemSelectPanel)}
