...
export function AccountFavorites({ favorites, updateFavorite }: Props) {
        ...
        vacations.map((vacation) => {
          return (
            <AccountFavoriteItem
              ...
              id={vacation.id}
              name={vacation.name}
              onRemove={() => updateFavorite(vacation.id)}
            />
          )
        })}
    </div>
...
type AccountFavoriteItemProps = {
  id: number
  name: string
  onRemove(id: number): void
}

export function AccountFavoriteItem({ id, name, onRemove }: AccountFavoriteItemProps) {
  return (
