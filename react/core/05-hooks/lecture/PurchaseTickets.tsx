...
export function App() {
  const [tickets, setTickets] = useState(3) // 0
  ...
  // const onpUpdate = useCallback((name: string, tickets: number) => {
    ...
    <form className="space-y-6">
      ...
      <div className="space-y-2">
        {attendees.map((number) => {
          return (
            <AddAttendeeFields key={number} ticketNumber={number + 1} setTickets={setTickets} />
          )
        })}
      </div>
    </form>
