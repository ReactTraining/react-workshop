...
export function DatePicker({
  ...
  function selectDate(date: D) {
    ...
    if (selectRange && selectedDates.length === 1) {
      const isAfter = date.isAfter(selectedDates[0], 'day')
      // Always ensure the dates in the array are in chronological order
      newSelectedDates = isAfter ? [selectedDates[0], date] : [date, selectedDates[0]]
    } else {
...
export function DatePickerCalendar({ offset = 0 }: DatePickerCalendarProps) {
  const { baseMonthFirst, selectDate, selectedDates, disablePastDays } = useContext(Context)
  ...
  // const selectedTimestamps = selectedDates.map((d) => d.format('YYYY-MM-DD'))
      ...
      <div data-datepicker-calendar-boxes="">
        {[...Array(daysInMonth).keys()].map((n) => {
          ...
          return (
            <button
              key={day}
              disabled={disablePastDays && isPast}
...
export function DatePickerMonth({ format = 'MMMM', offset = 0, ...props }: DatePickerMonthProps) {
